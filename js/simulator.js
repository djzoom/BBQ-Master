...
window.SmokerSim.simulator = (function () {
  ...
  function create(inputs) {
    ...
    return {
      ...
      weightLb: inputs.weightLb || 10, areaM2: 0.07,
      tAmbC: C.fToC(inputs.tAmbF != null ? inputs.tAmbF : 70),
      humidityPct: inputs.humidityPct || 50, windMph: inputs.windMph || 2,
      equipment: inputs.equipment || 'offset', elevationFt: inputs.elevationFt || 0,
      tSimMin: 0, T: HD.initProfile(n, tInitC),
      w: 0.75, wSurface: 1.0, C: 0.0,
      smoke: { good: 0, bad: 0, ringFrozen: false }, smokeRingGoodAtCutoff: 0,
      crust: 0,
      // Thermal damage + creosote (oxygen-starved combustion).
      burnLevel: 0, creosote: 0,
      // Bark chemistry: time-weighted Maillard quality (peaks 110–150 °C
      // surface, drops past 160 °C) + sugar scorch above 160 °C.
      barkQuality: 1.0, sugarBurn: 0,
      // Lid-open oxygen surge: builds while open, drains over ~4 min.
      oxBoost: 0,
      kSafety: 0.0,
      ...
      wrapState: inputs.wrapType || 'none', phase: 'bark_build',
  ...
  function step(state, dtSec) {
    ...
    for (var s = 0; s < nSub; s++) {
      // Combustion saturation — finite oxygen, creosote builds when starved.
      ...
      var qFire = oxStarved > 0 ? qFireRaw * oxCapacity / activeCoals : qFireRaw;
      if (oxStarved > 0) state.creosote = Math.min(1, (state.creosote || 0)
        + (oxStarved / oxCapacity) * (subDt / 3600));

      var hEff = C.H_BASE * (1 + 0.05 * state.windMph);
      if (state.lidOpenSec > 0) hEff *= C.H_LID_OPEN_MULT;

      // Lid-open oxygen surge — transient overshoot after close.
      if (state.lidOpenSec > 0) {
        state.oxBoost = Math.min(1, (state.oxBoost || 0) + subDt / 60);
      } else if ((state.oxBoost || 0) > 0) {
        qFire *= 1.0 + 0.40 * state.oxBoost;
        state.oxBoost = Math.max(0, state.oxBoost - subDt / 240);
      }

      var qToMeat = PM.qToMeat(state.tPitC, state.T[0], hEff, state.areaM2);
      state.tPitC = PM.step(state.tPitC, qFire, qToMeat, state.tAmbC, subDt, { lidOpen: state.lidOpenSec > 0 });
      ...
      // Wet-bulb stall cap — surface locks at 72 °C while wet, ramps to boil as w drains.
      var WET_BULB_C = 72;
      ...
      // Foil vs paper steam dynamics — trapped wrap re-condenses on outer skin.
      var rehydrate = 0;
      if (state.wrapState === 'aluminum_foil' && state.T[0] < 99) rehydrate = 0.30;
      else if (state.wrapState === 'butcher_paper' && state.T[0] < 99) rehydrate = 0.10;
      else if (state.wrapState === 'foil_boat' && state.T[0] < 99) rehydrate = 0.18;
      if (rehydrate > 0) state.wSurface = Math.min(1.0, state.wSurface + rehydrate * (subDt / 3600));
      ...
      // Thermal damage — sustained 250 °C+ pit reaches full burn in ~4 h.
      var burnRate = 0;
      ...
      var surfC = state.T[0];
      // Crust accumulator — surface in 95–180 °C × dryness × wrap_open.
      if (surfC >= 95 && surfC <= 180) {
        ...
        var wrapOpen = state.wrapState === 'aluminum_foil' ? 0.10
                     ...
                     : state.wrapState === 'foil_boat'     ? 0.70 : 1.0;
        state.crust = Math.min(1, (state.crust || 0) + dryness * wrapOpen * (subDt / 60) * 0.012);
      }
      // Bark chemistry — time-weighted Maillard window quality (peak 130 °C).
      if (surfC >= 95 && surfC <= 180) {
        var sweet = surfC >= 110 && surfC <= 150 ? 1.0
                  : surfC > 150 ? Math.max(0, 1 - (surfC - 150) / 30)
                  : Math.max(0, (surfC - 95) / 15);
        state.barkQuality = (state.barkQuality || 1) * 0.999 + sweet * 0.001;
      }
      // Sugar / rub scorch above 160 °C surface.
      if (surfC > 160) state.sugarBurn = Math.min(1, (state.sugarBurn || 0) + (surfC - 160) * 0.00005 * subDt);
  ...
  function ignite(state, n) { FM.refuel(state.coals, n, state.tSimMin, C.COAL_P_PEAK, C.COAL_TAU_BURN_MIN); state.eventLog.push({ t: state.tSimMin, kind: 'ignite', n: n }); }
  function refuel(state, n) { FM.refuel(state.coals, n, state.tSimMin, C.COAL_P_PEAK, C.COAL_TAU_BURN_MIN); state.eventLog.push({ t: state.tSimMin, kind: 'refuel', n: n }); }
  function addWood(state, massKg, species) { state.woodAdds.push({ tAddMin: state.tSimMin, mass: massKg, species: species || 'oak' }); state.eventLog.push({ t: state.tSimMin, kind: 'wood', mass: massKg, species: species }); }
  function damper(state, pct) { state.damperPct = Math.max(0, Math.min(100, pct)); state.eventLog.push({ t: state.tSimMin, kind: 'damper', pct: pct }); }
  function wrap(state, type) { state.wrapState = type; state.phase = 'push'; state.eventLog.push({ t: state.tSimMin, kind: 'wrap', type: type }); }
  function openLid(state, seconds) { state.lidOpenSec = (state.lidOpenSec || 0) + seconds; state.tPitC -= C.T_PIT_LID_DROP_C * Math.min(seconds / 60, 1); state.eventLog.push({ t: state.tSimMin, kind: 'lid', seconds: seconds }); }
  function spritz(state, volumeMl) { var v = volumeMl || 30; state.wSurface = Math.min(1, (state.wSurface || 0) + 0.15); state.spritzEndMin = state.tSimMin + 2; state.spritzCount = (state.spritzCount || 0) + 1; state.eventLog.push({ t: state.tSimMin, kind: 'spritz', volume: v }); }
  function tallow(state, volumeMl) { var v = volumeMl || 30; state.w = Math.min(1, state.w + 0.05); state.wSurface = Math.min(1, (state.wSurface || 0) + 0.20); state.eventLog.push({ t: state.tSimMin, kind: 'tallow', volume: v }); }
  function pull(state) { state.phase = 'rest'; state.tPullC = state.T[state.T.length - 1]; state.wAtPull = state.w; state.tPullMin = state.tSimMin; state.eventLog.push({ t: state.tSimMin, kind: 'pull' }); }
  function slice(state, restMethod) {
    var tRest = (state.tPullMin != null) ? Math.max(0, state.tSimMin - state.tPullMin) : (state.tRestMin || 0);
    state.tRestMin = tRest; state.phase = 'slice';
  ...
  function removeEvent(state, idx) {
    ...
    if (e.kind === 'refuel' || e.kind === 'ignite') state.coals = state.coals.filter(function (c) { return c.tIgniteMin !== e.t; });
    else if (e.kind === 'wood') state.woodAdds = state.woodAdds.filter(function (w) { return w.tAddMin !== e.t; });
    else if (e.kind === 'wrap') {
      var prev = 'none';
      for (var i = idx - 1; i >= 0; i--) if (state.eventLog[i].kind === 'wrap') { prev = state.eventLog[i].type; break; }
      state.wrapState = prev;
    } else if (e.kind === 'damper') {
      var prevD = 60;
      for (var k = idx - 1; k >= 0; k--) if (state.eventLog[k].kind === 'damper') { prevD = state.eventLog[k].pct; break; }
      state.damperPct = prevD;
    } else if (e.kind === 'pull' || e.kind === 'slice') {
      state.phase = 'bark_build'; state.wRetained = null; state.tPullC = null; state.tPullMin = null;
    }
